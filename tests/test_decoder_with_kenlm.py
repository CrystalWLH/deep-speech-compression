...
chars_labels = table.lookup(features)

join_expected = tf.reduce_join(chars_labels, separator = '', axis = 1)

print(join_expected.eval(session = sess))

split_expected = tf.string_split(join_expected)

split_expectd_no_skip = tf.string_split(join_expected, skip_empyt = False)

dense_expected = tf.sparse_to_dense(split_expected.indices,
                               split_expected.dense_shape,
                               split_expected.values)

print(dense_expected.eval(session = sess))

dense_expected_no_skip = tf.sparse_to_dense(split_expectd_no_skip.indices,
                               split_expectd_no_skip.dense_shape,
                               split_expectd_no_skip.values)

print(dense_expected_no_skip.eval(session = sess))
